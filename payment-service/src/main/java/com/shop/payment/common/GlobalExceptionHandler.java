...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiError> api(PaymentApiException ex) {
    ...
    ResponseEntity<ApiError> integrity(DataIntegrityViolationException ex) {
        return error(HttpStatus.CONFLICT, "CONSTRAINT_VIOLATION", "Payment data violates a database constraint", List.of());
    }
    ...
    ResponseEntity<ApiError> unexpected(Exception ex) {
        log.error("Unhandled payment API exception", ex);
