...
- JUnit 5 + MockMvc + Testcontainers 1.21.4
...
Integration tests use Testcontainers 1.21.4 and start an isolated `postgres:17`
...
curl -X POST \
  -F 'file=@./product.png' \
  'http://localhost:8080/api/v1/products/<product-id>/images'
```
...
curl -i 'http://localhost:8080/api/v1/products?page=0&size=20&sort=name,asc'
curl -i 'http://localhost:8080/api/v1/products?categoryId=<category-id>&page=0&size=20&sort=name,asc'
```
