    ...
    public OrderResponse create(CreateOrderRequest request, String idempotencyKey, String bearerToken) {
        try {
            ...
            if (status == 400) throw new com.shop.cart.service.CartApiException(
                    org.springframework.http.HttpStatus.BAD_REQUEST, "ORDER_REJECTED", "Order rejected checkout inputs");
            if (status == 409) throw new com.shop.cart.service.CartApiException(
                    org.springframework.http.HttpStatus.CONFLICT, "CHECKOUT_CONFLICT", "Order could not be created because inventory or checkout state conflicted");
