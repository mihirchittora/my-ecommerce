...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiError> api(CartApiException ex) {
        return error(ex.getStatus(), ex.getCode(), ex.getMessage(), List.of());
    }

    @ExceptionHandler(CartDependencyException.class)
    ResponseEntity<ApiError> dependency(CartDependencyException ex) {
        return error(ex.status(), ex.code(), ex.getMessage(), List.of());
    }
    ...
    ResponseEntity<ApiError> optimistic(ObjectOptimisticLockingFailureException ex) {
        return error(HttpStatus.CONFLICT, "CONCURRENT_UPDATE", "Cart was changed by another request; retry with the latest cart", List.of());
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    ResponseEntity<ApiError> integrity(DataIntegrityViolationException ex) {
        return error(HttpStatus.CONFLICT, "CONSTRAINT_VIOLATION", "Cart data violates a database constraint", List.of());
    }

    @ExceptionHandler(AccessDeniedException.class)
    ResponseEntity<ApiError> accessDenied(AccessDeniedException ex) {
        return error(HttpStatus.FORBIDDEN, "FORBIDDEN", "Insufficient permission", List.of());
    }

    @ExceptionHandler(Exception.class)
    ResponseEntity<ApiError> unexpected(Exception ex) {
        log.error("Unhandled Cart API exception", ex);
