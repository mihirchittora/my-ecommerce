...
public class SecurityConfig {
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http, ObjectProvider<JwtDecoder> decoderProvider,
                                            OrderServiceAuthenticationFilter serviceFilter) throws Exception {
        http.csrf(csrf -> csrf.disable()).sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .addFilterBefore(serviceFilter, org.springframework.security.oauth2.server.resource.web.BearerTokenAuthenticationFilter.class);
        if (!enabled) {
        ...
        http.authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers(HttpMethod.POST, "/api/v1/shipping/webhooks/**").permitAll()
                .requestMatchers("/internal/**").hasAuthority("SERVICE_ORDER")
                .requestMatchers(HttpMethod.GET, "/api/v1/shipments/my").authenticated()
                .requestMatchers(HttpMethod.GET, "/api/v1/shipments/*", "/api/v1/shipments/*/tracking").authenticated()
                .requestMatchers(HttpMethod.GET, "/api/v1/fulfillments/*", "/api/v1/fulfillments/order/*").authenticated()
                .requestMatchers(HttpMethod.GET, "/api/v1/shipments", "/api/v1/fulfillments").hasAuthority("SHIPPING_READ")
                .requestMatchers(HttpMethod.POST, "/api/v1/shipments/*/cancel").hasAuthority("SHIPPING_CANCEL")
                .requestMatchers(HttpMethod.POST, "/api/v1/shipments/*/order-notification/retry").hasAuthority("SHIPPING_MANAGE")
                .requestMatchers(HttpMethod.POST, "/api/v1/shipments").hasAuthority("SHIPPING_CREATE")
