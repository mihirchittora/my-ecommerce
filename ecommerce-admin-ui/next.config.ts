...
const paymentApiUrl = process.env.NEXT_PUBLIC_PAYMENT_API_URL ?? "http://localhost:8087";
const shippingApiUrl = process.env.NEXT_PUBLIC_SHIPPING_API_URL ?? "http://localhost:8088";
...
const nextConfig: NextConfig = {
  ...
  async rewrites() {
    return [
      ...
      {
        source: "/backend/payment/:path*",
        destination: `${paymentApiUrl}/api/:path*`,
      },
      {
        source: "/backend/shipping/:path*",
        destination: `${shippingApiUrl}/api/:path*`,
      },
    ];
  },
};
