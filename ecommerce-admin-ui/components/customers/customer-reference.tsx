...
export function CustomerReference({ customerId }: { customerId: string }) {
  ...
  const customer = useCustomerByAuthUserId(customerId, canReadCustomer);
  ...
  return <div className="min-w-0" title={customerId}>
    {canReadCustomer ? <Link href={customer.data ? `/customers/${customer.data.id}` : "#"} className="block max-w-[15rem] truncate text-sm font-semibold text-primary hover:underline">{name}</Link> : <span className="block text-sm font-medium text-slate-700">Customer details unavailable</span>}
